...
for file in RCR_files:
    RCR = np.concatenate((RCR, np.load(file)[50000:,0:4]))
...
model = keras.models.load_model(f'Code/h5_models/{round}_trained_CNN_1l-10-8-10_do0.5_fltn_sigm_valloss_p4_measNoise0-20k_0-5ksigNU-Scaled_shuff_monitortraining_0_13.h5')
...
if plotRandom == True:
    for iE, Noi in enumerate(Noise):
        ...
        if output > 0.95:
           ...
           plotTrace(Noi, f"Noise {iE}, Output {output:.2f}",f"Code/data/4thpass/Noise_{iE}_Output_{output:.2f}_Station{station}.png")
    for iE, rcr in enumerate(RCR):
        if iE % 1000 == 0:
            output = 1 - prob_RCR[iE][0]
            if output > 0.95:
                plotTrace(rcr, f"RCR {iE}, Output {output:.2f}",f"Code/data/4thpass/RCR_{iE}_Output_{output:.2f}_Station{station}.png")
...
plt.savefig(path+f'MaxAmps_Output Station{station}.png', format='png')
plt.clf()
#plt.show()
...
#plt.show()
plt.savefig(path+f'Station{station}AnalysisOutput.png', format='png')
